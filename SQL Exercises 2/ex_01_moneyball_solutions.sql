...
-- Solution one, using nested queries
...
SELECT first_name, last_name FROM (
    SELECT players.first_name, players.last_name, salaries.salary/performances.H AS "dollars per hit", salaries.salary/performances.RBI AS "dollars per RBI" FROM players
    JOIN performances ON performances.player_id = players.id
    JOIN salaries ON salaries.player_id = players.id AND salaries.year = performances.year
    WHERE salaries.year = 2001 AND performances.H > 0 
    GROUP BY players.id
